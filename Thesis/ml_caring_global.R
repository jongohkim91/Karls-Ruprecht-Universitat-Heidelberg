...
#restricting the treatment group to nursing and caring sector
dataset_treated <- dataset %>% 
  filter((w08_2>=86 & w08_2<90) & mw==1)
dataset_control <- dataset %>% 
  filter(mw==0)
dataset <- rbind.data.frame(dataset_treated, dataset_control)
rm(dataset_treated, dataset_control)
...
dataset <- dataset %>% rename(equipment_year=sachanlageneurjahr)
...
total_var <- c( #choosing the same variables as in split!
  "crefonummer",
  "year",
  "mw",
  "ln_emp_nn1", 
  "ln_bilanzsumme_nn1",
  "ln_anlagevermögen_nn1",
  "ln_intangible_assets", 
  "ln_sachanlagen_nn1",
  "ln_umlaufvermögen_nn1",
  "ln_vor_nn1",
  "ln_total_equity",           
  "ln_verbindlichkeiten_nn1",
  "company_type",
  "solvency_status",
  "cap_int_nn1",
  "inv_int_nn1",
...
save.image(file = "cleaned_west_germany2018_global.RData")
load("cleaned_west_germany2018_global.RData")
"=================================================================================="
...
non_para_data <- og_data %>%
  select(total_var, ln_sachanlage_pemp_nn1) 
...
#median imputation by firm
for(i in 1: length(unique(sort(non_para_data$crefonummer)))){
  temp_data <- non_para_data %>% #data by each firm
    filter(crefonummer == unique(sort(non_para_data$crefonummer))[i] )
  
  temp_data <- centralImputation(temp_data) #knn imputation by firm
  
  firm_idx <- non_para_data$crefonummer %in% unique(sort(non_para_data$crefonummer))[i]
  non_para_data[firm_idx, ] <- temp_data
}
rm(temp_data)
...
#removing ln_vor_nn1 & ln_intangible_assets as still some missing values exist after imputation process
non_para_data <- non_para_data %>%
  select(-ln_intangible_assets, -ln_vor_nn1)
...
#training_set_non_para = subset(non_para_data, split == TRUE) %>% 
#  as.data.frame()
#test_set_non_para = subset(non_para_data, split == FALSE) %>% 
#  as.data.frame()
...
#folds = createFolds(training_set$mw, k = 10)
...
"=================================================================================="
#writing temporary rds file for time efficiency
save.image(file = "knn_global.RData")
load("knn_global.RData")
"=================================================================================="

#removing some redundant columns to reduce required memory
all_var <- c(
  "crefonummer",
  "year",
  "mw",
...
matching <- function(method_input="logit"){
  ...
  if(stri_cmp_eq(method, "logit")){
    ...
    cv = lapply(folds, function(x) {
      ...
      if(1 %in% y_pred){
        cm = table(test_fold[, mw_idx], y_pred == 1)
        accuracy = (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] + cm[1,2] + cm[2,1])
        return(accuracy)  
      }
      if(!(1 %in% y_pred)){
...
"=================================================================================="
#writing temporary rds file for time efficiency
#save.image(file = "matching_global.RData")
load("matching_global.RData")
"=================================================================================="
...
"=================================================================================="
#writing temporary rds file for time efficiency
save.image(file = "result_global.RData")
load("result_global.RData")
"=================================================================================="
"================================================================================================================="
"=========================== RESULT ANALYSIS ============================================================================"
"================================================================================================================="
...
write.csv(analytical_result_demeaned_svm, "global_pooled_OLS_svm.csv")
#write.csv(analytical_result_demeaned_ksvm, "pooled_OLS_ksvm.csv")
write.csv(analytical_result_demeaned_logit, "global_pooled_OLS_logit.csv")
#write.csv(analytical_result_demeaned_rf, "global_pooled_OLS_rf.csv")
#write.csv(analytical_result_demeaned_rf_non_para, "global_pooled_OLS_rf_non_para.csv")
write.csv(analytical_result_demeaned_xgboost, "global_pooled_OLS_xgboost.csv")
write.csv(analytical_result_demeaned_xgboost_non_para, "global_pooled_OLS_xgboost_non_para.csv")
"=================================================================================="
#writing temporary rds file for time efficiency
save.image(file = "analysis_global.RData")
load("analysis_global.RData")
"=================================================================================="